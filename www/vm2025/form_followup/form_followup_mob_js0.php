...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_followup_mob']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_followup_mob']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_followup_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{ 
   ...
   function nm_gp_print_conf(tp, cor, SC_module_export, password, ajax, str_type, bol_param)
   {
       ...
       {
           window.open('<?php echo$this->Ini->path_link . "" . SC_dir_app_name('form_followup_mob') . "/form_followup_mob_iframe_prt.php?path_botoes=" . $this->Ini->path_botoes . "&script_case_init=" . $this->form_encode_input($this->Ini->sc_page); ?>&apl_prt=form_followup_mob.php&opcao=print&cor_print=' + cor,'','location=no,menubar,resizable,scrollbars,status=no,toolbar');
       }
   }
