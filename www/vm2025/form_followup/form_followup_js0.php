...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_followup']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_followup']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_followup"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_followup']['where_detal']))
  {
?>  
...
{ 
   ...
   function nm_gp_print_conf(tp, cor, SC_module_export, password, ajax, str_type, bol_param)
   {
       ...
       {
           window.open('<?php echo$this->Ini->path_link . "" . SC_dir_app_name('form_followup') . "/form_followup_iframe_prt.php?path_botoes=" . $this->Ini->path_botoes . "&script_case_init=" . $this->form_encode_input($this->Ini->sc_page); ?>&apl_prt=form_followup.php&opcao=print&cor_print=' + cor,'','location=no,menubar,resizable,scrollbars,status=no,toolbar');
       }
   }
...
{
   nm_field_disabled("data=disabled", "");
...
function scCssBlur(oHtmlObj)
{
 ...
 function nm_field_disabled(Fields, Opt) {
  ...
  for (i=0; i < Field.length; i++)
  {
     ...
     if (F_name == "cod_vend")
     {
        $('input[name="cod_vend_autocomp"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('input[name="cod_vend_autocomp"]').addClass("scFormInputDisabled");
        }
        else {
            $('input[name="cod_vend_autocomp"]').removeClass("scFormInputDisabled");
        }
        $('input[id="cod_vend_autocomp_cap"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('#cod_vend_autocomp_cap').hide();
        }
        else {
            $('#cod_vend_autocomp_cap').show();
        }
     }
     if (F_name == "data")
     {
