...
function bootstrapMobile(fnCall) {
    var app = getAppData();
    var fnCallStr = (typeof fnCall === 'undefined') ? 'none' : fnCall;
    ...
    if (isMobile() && app.improvements) {
        ...
        var rChartExport = $('#sc-id-combination-table');
        var rChartExport = false;
        ...
        if (rDynamicSearch) rDynamicSearch.openInModalPane({
            ...
            onReady: function (paneContent, openButton, closeButton) {
                ...
                applyBtn.on('click.finishProcessing', function (e) {
                    closeButton.click();
                });
            },
            // beforeReady: function (paneContent, openButton, closeButton) {
            ...
            var observer = new MutationObserver(function(mutations) {
                mutations.forEach(function(mutation) {
                    ...
                    nodes.forEach(function(node) {
                        if (node.className == "select2-results__option") {
                            found = true;
                        }
                    });
                    if (found) {
        ...
        if (typeof nmAjaxProcOn === 'function' && typeof $nmAjaxProcOn !== 'function') {
            ...
            window.nmAjaxProcOn = function () {
                ...
                if (typeof $('.modal-pane-container.active')[0] !== typeof undefined) {
                    if (typeof $('.modal-pane-container.active[nm-modalpane-holdajax]')[0] === typeof undefined) {
                        window.history.back();
                    }
                }
                $nmAjaxProcOn();
...
function scBtnGrpShow(sGroup) {
    return scBtnGrpShowMobile(sGroup);
}
...
function scBtnGrpHide(sGroup) {
    return scBtnGrpHideMobile(sGroup);
}
...
function toggleToolbar(e, forceClose) {
    ...
    var app = appData.appType;
    var elFind = findElements();
    var toolBarTop = elFind.toolBarTop;
    var scToolbar = elFind.scToolbar;
    var scHeadOptions = elFind.scHeadOptions;
    var scToolbarPadding = elFind.scToolbarPadding;
    var scToolbarPaddingParent = elFind.scToolbarPaddingParent;
    ...
    if (stState) {
        ...
        if (!$('.overlayToolbar')[0]) {
            ...
            if (scToolbarPadding.outerWidth() > scToolbarPaddingParent.outerWidth()) {
...
function findElements() {
    ...
    var ret_data = {
        ...
        scToolbar: $('body'),
        scHeadOptions: $('body'),
        scToolbarPadding: $('body'),
        scToolbarPaddingParent: $('body'),
    };


    switch (app) {
        case 'grid':
            ret_data.toolBarTop = $('#sc_grid_toobar_top').parent();
            ret_data.scToolbar = $('.scGridToolbar');
            ret_data.scHeadOptions = $('#sc_grid_head').find('.headerOptions');
            ret_data.scToolbarPadding = ret_data.toolBarTop.find('.scGridToolbarPadding');
            ret_data.scToolbarPaddingParent = ret_data.toolBarTop;
            break;
        case 'form':
            ret_data.toolBarTop = $('.scFormToolbar').parent();
            ret_data.scToolbar = $('.scFormToolbar');
            ret_data.scHeadOptions = $('td.scFormHeader').find('.headerOptions');
            ret_data.scToolbarPadding = ret_data.toolBarTop.find('.scFormToolbarPadding');
            ret_data.scToolbarPaddingParent = ret_data.toolBarTop;
            break;
        case 'search':
            ret_data.toolBarTop = $('.scFilterToolbar').parent();
            ret_data.scToolbar = $('.scFilterToolbar');
            ret_data.scHeadOptions = $('.scFilterHeader').find('.headerOptions');
            ret_data.scToolbarPadding = ret_data.toolBarTop.find('.scFilterToolbarPadding');
            ret_data.scToolbarPaddingParent = ret_data.toolBarTop;
            break;
        case 'detail':
        case 'summary':
            ret_data.toolBarTop = $('.scGridTabelaTd > .scGridToolbar').parent();
            ret_data.scToolbar = $('.scGridTabelaTd > .scGridToolbar');
            ret_data.scHeadOptions = $('.scGridTabelaTd > .scGridHeader').find('.headerOptions');
            ret_data.scToolbarPadding = ret_data.toolBarTop.find('.scGridToolbarPadding');
            ret_data.scToolbarPaddingParent = ret_data.toolBarTop;
            break;
    }
        ...
        case 'grid':
            ...
            if (appData.toolbarOrientation == 'H') {
                if (scToolbarPadding.outerWidth() > scToolbarPaddingParent.outerWidth()) {
                    ...
                    $('#sc_grid_toobar_top').append('<div id="slide_signal" style="display: none;"><div class="bnc_arrow">&#x2039;</div></div>');
                }
            ...
            if (appData.bottomToolbarFixed) {
                if ($('#sc_grid_toobar_bot')[0]) {
                    ...
                    mobileAddCSS('' +
                        ...
                        '}' +
                        '#sc_grid_toobar_bot, #sc_filter_toolbar_bot { ' +
                        'position: ' + 'fixed;' +
                        'z-index: ' + '4;' +
                        '}'
                    );
        ...
        case 'search':
            ...
            if (appData.toolbarOrientation == 'H') {
                if (scToolbarPadding.outerWidth() > scToolbarPaddingParent.outerWidth()) {
                    ...
                    $('.scFilterToolbar:not(#sc_filter_toolbar_bot *)').append('<div id="slide_signal" style="display: none;"><div class="bnc_arrow">&#x2039;</div></div>');
                }
