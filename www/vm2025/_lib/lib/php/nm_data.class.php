...
*/
    ...
    function nm_calc_hora(&$hora, &$minuto, &$segundo, &$dia, $horas_calc = 0, $minutos_calc = 0, $segundos_calc = 0, $direcao = '+')
    {
        //--- sem operacao
        if (0 >= $horas_calc && 0 >= $minutos_calc && 0 >= $segundos_calc) {
        ...
        $minuto        = (int) $minuto;
        $segundo       = (int) $segundo;
        $horas_calc    = (int) $horas_calc;
        $minutos_calc  = (int) $minutos_calc;
        $segundos_calc = (int) $segundos_calc;
        $int_dir       = ('+' == $direcao) ? 1 : -1;

        $segundo += $segundos_calc * $int_dir;
        if (0 > $segundo) {
            $segundo = 60 + $segundo;
            $minuto--;
        } elseif (60 <= $segundo) {
