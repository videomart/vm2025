...
function nm_get_api_gateways_deep($api = '')
{
    ...
    $arr_apis['payment']['paypal_rest'] = [
        ...
        [
//            'token' => ['placeholder' => 'Your Auth token', 'type' => 'paypal_auth_code'],
            'testMode' => ['options' => ['FALSE', 'TRUE'], 'type' => 'select'],
        ]
    ];
    ...
    $arr_apis['reporting']['google_sheets'] = [
        [
            "dir_name" => ['placeholder' => 'Folder name'],
            "dir_id"   => ['placeholder' => 'Folder id', 'type' => 'hidden'],
            "app_name" => ['placeholder' => 'Your App Name'],
            "auth_code" => ['placeholder' => 'Click in button and insert Auth Code generated', 'type' => 'gd_auth_code'],
        ],
        [
    ...
    switch (strtolower($arr_settings['settings']['gateway'])) {
        ...
        case 'paypal_rest':
            ...
            $instance = $gateway->initialize([
                ...
                'token' => isset($arr_settings['settings']['token']) ? $arr_settings['settings']['token'] : '',
                'testMode' => isset($arr_settings['settings']['testMode']) ? $arr_settings['settings']['testMode'] : false,
            ]);
...
function api_google_get_client($profile, $app_name, $json_oauth, $auth_code = '', $token = '', $scope = '', $return_token = false)
{
    ...
    if(isset($_SERVER['HTTP_REFERER'])) {
        ...
        try{
            $json_validate = json_decode($json_oauth, true);
...
function sc_googlesheets_create($arr_settings, $filename, $parent_id=null){
    ...
    $parent_id = isset($arr_settings['settings']['dir_id']) && !empty($arr_settings['settings']['dir_id']) && is_null($parent_id) ? $arr_settings['settings']['dir_id'] : $parent_id;
    if(!is_null($parent_id)){
        return sc_googledrive_file_create($arr_settings,$filename,$parent_id,'application/vnd.google-apps.spreadsheet',['fields' => 'id']);
    }else{
...
function sc_googledrive_folder_create($arr_settings,$dirname=null){
    $client = api_google_get_client('', $arr_settings['settings']['app_name'], $arr_settings['settings']['json_oauth'],
        isset($arr_settings['settings']['auth_code']) ? $arr_settings['settings']['auth_code'] : '',
        isset($arr_settings['settings']['token_code']) ? $arr_settings['settings']['token_code'] : '',
        'drive',
        isset($arr_settings['settings']['return_token']) ? $arr_settings['settings']['return_token'] : false);
    $instance   =  new Google_Service_Drive($client);
    return sc_googledrive_folder_search($instance,$dirname);
}
function sc_googledrive_folder_search($instance, $path=null, $rootParentId = null) {
    $parentId = null;
    if(!is_null($path)){
        $folders = str_replace('\\','/',trim($path));
        $folders = explode('/', trim($folders, '/'));
        $parentId = $rootParentId;
        foreach ($folders as $folderName) {
            $query = sprintf( "mimeType = 'application/vnd.google-apps.folder' and name = '%s' and trashed = false", addslashes($folderName));
            if ($parentId) {$query .= " and '{$parentId}' in parents";}
            $search = $instance->files->listFiles([
                'q' => $query,
                'spaces' => 'drive',
            ...
            if (count($search->getFiles()) > 0) {
                ...
                $createdFolder = $instance->files->create($newFolder, ['fields' => 'id']);
...
function sc_googledrive_file_create($arr_settings,$filename,$parent_id=null,$mimeType='',$options=[]){
    $client = api_google_get_client('', $arr_settings['settings']['app_name'], $arr_settings['settings']['json_oauth'],
        isset($arr_settings['settings']['auth_code']) ? $arr_settings['settings']['auth_code'] : '',
        isset($arr_settings['settings']['token_code']) ? $arr_settings['settings']['token_code'] : '',
        'drive',
    ...
    $file = $instance->files->create($file, $options);
    return $file->id;
}