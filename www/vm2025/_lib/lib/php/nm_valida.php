...
/*--- mantis 0025919 ---*/
            /* Check for well-formed parentheses */
            if (substr_count($valorIn, '(') !== substr_count($valorIn, ')')) {
                return false;
            }
            /* Regex to capture (AI) + value */
            preg_match_all('/\((\d{2,4})\)([^()]*)/', $valorIn, $matches, PREG_SET_ORDER);
            if (empty($matches)) {
                return false;
            }
                ...
                switch ($ai) {
                    case '01': // GTIN-14
                        if (!preg_match('/^\d{14}$/', $val)) {
                            return false; /* AI (01): GTIN must have 14 digits */
                        }
                        if (!$this->validateGTIN($val)) {
                            return false; /* invalid check digit */
                        }
                        break;
                    case '17': // Expiration date YYMMDD
                        if (!preg_match('/^\d{6}$/', $val)) {
                            return false; /* AI (17): date must have 6 digits (YYMMDD) */
                        }
                        if (!$this->validateDateYYMMDD($val)) {
                            return false; /* invalid date */
                        }
                        break;
                    case '10': // Batch number (alphanumeric, variable)
                        if (strlen($val) > 20) {
                            return false; /* batch number with more than 20 characters */
                        }
                        break;
                    default:
                        break;
...
/*------*/
      ...
      function validateGTIN($gtin) {
          ...
          $dv = (10 - ($soma % 10)) % 10;
          return $dv == (int)$gtin[13];
      }
      function validateDateYYMMDD($dt) {
          $Y = intval(substr($dt, 0, 2)) + 2000;
          $m = intval(substr($dt, 2, 2));
          $d = intval(substr($dt, 4, 2));
...
*/
            if (!preg_match_all('/^[A-Z0-9]+$/',$CnpjIn)) {
                return false ;
            }
/*------*/
...
/*    ================================================================
                              HORA
      ================================================================ */
      function Hora($HoraIn, $HoraFormat, $HoraMin = '', $HoraMax = '')
      {
            if (nm_reg_prod() != "NmScriptCaseAplOk")
            {
                return false;
            ...
            $hora_teste_min  = ($Hora != ""  ?  $Hora : "00") ;
            $hora_teste_min .= ($Min != ""  ?  $Min : "00") ;
            $hora_teste_min .= ($Seg != ""  ?  $Seg : "00") ;
            $hora_teste_max  = ($Hora != ""  ?  $Hora : "23") ;
            $hora_teste_max .= ($Min != ""  ?  $Min : "59") ;
            $hora_teste_max .= ($Seg != ""  ?  $Seg : "59") ;
            if (!empty($HoraMin) && $hora_teste_min < $HoraMin)
            {
                $Erro .= " Hora m�nima";
            }
            if (!empty($HoraMax) && $hora_teste_max > $HoraMax)
            {
                $Erro .= " Hora m�xima";
            }
...
/*    ================================================================
                              DATA HORA
      ================================================================ */
    function DataHora($DataIn, $DataFormat, $HoraIn, $HoraFormat, $DataMin, $DataMax, $HoraMin = '', $HoraMax = '')
    {
        if (nm_reg_prod() != "NmScriptCaseAplOk") {
            return false;
        }
        ...
        // data hora minima/maxima
        $datahora_teste_min = ($Ano != "" ? $Ano : "0000");
        $datahora_teste_min .= ($Mes != "" ? $Mes : "00");
        $datahora_teste_min .= ($Dia != "" ? $Dia : "00");
        $datahora_teste_min .= ($Hora != ""  ?  $Hora : "00");
        $datahora_teste_min .= ($Min != ""  ?  $Min : "00");
        $datahora_teste_min .= ($Seg != ""  ?  $Seg : "00");
        $datahora_teste_max = ($Ano != "" ? $Ano : "0000");
        $datahora_teste_max .= ($Mes != "" ? $Mes : "00");
        $datahora_teste_max .= ($Dia != "" ? $Dia : "00");
        $datahora_teste_max .= ($Hora != ""  ?  $Hora : "23");
        $datahora_teste_max .= ($Min != ""  ?  $Min : "59");
        $datahora_teste_max .= ($Seg != ""  ?  $Seg : "59");
        if (!empty($DataMin) && !empty($HoraMin) && $datahora_teste_min < $DataMin . $HoraMin) {
            $Erro .= " Data m�nima";
        }
        if (!empty($DataMax) && !empty($HoraMax) && $datahora_teste_max > $DataMax . $HoraMax) {
            $Erro .= " Data m�xima";
        }
