...
{
    global $root, $Db, $prefixo;

    $dados    = array();
    $estado   = "";
    $comando  = "SELECT uf, cidade, cep FROM ". $prefixo ."cidade order by uf, cidade";
    $rs_local = $Db->Execute($comando);
    if ($rs_local === false && !$rs_local->EOF)
    {
        echo "<br>*** Erro ao acessar tabela ". $prefixo ."cidade: " . $Db->ErrorMsg();
        exit ;
    }
    $arr_ceps = array();
    while (!$rs_local->EOF)
    {
        ...
        $dados[5] = "";   // Longitude
        if (!empty($dados[2])) {
            $comando  = "SELECT latitude, longitude FROM ". $prefixo ."geo WHERE cep = '" . $dados[2] . "'";
            $rs_geo = $Db->Execute($comando);
            if ($rs_geo === false && !$rs_geo->EOF) {
                echo "<br>*** Erro ao acessar tabela ". $prefixo ."geo (Localidades): " . $Db->ErrorMsg();
                exit ;
            }
            if (!$rs_geo->EOF) {
...
function conv_fx_estados()
{
    ...
    $comando = "SELECT uf, cep_inicial, cep_final FROM ". $prefixo ."faixa_cidades order by uf, cep_inicial";
    $rs_fx_est = $Db->Execute($comando);
    if ($rs_fx_est === false && !$rs_fx_est->EOF)
    {
        echo "<br>*** Erro ao acessar tabela ". $prefixo ."faixa_cidades: " . $Db->ErrorMsg();
        exit ;
    }
    while (!$rs_fx_est->EOF)
    {
...
function conv_fx_localidades()
{
    ...
    $comando = "SELECT ". $prefixo ."cidade.uf, ". $prefixo ."cidade.cidade, ". $prefixo ."faixa_cidades.cep_inicial, ". $prefixo ."faixa_cidades.cep_final FROM
   ". $prefixo ."cidade INNER JOIN ". $prefixo ."faixa_cidades ON ". $prefixo ."cidade.id_cidade = ". $prefixo ."faixa_cidades.id_cidade
   where ". $prefixo ."cidade.cep is null OR ". $prefixo ."cidade.cep = ''
   order by ". $prefixo ."cidade.uf, ". $prefixo ."cidade.cidade";
...
function conv_logradouros()
{
    ...
    $comando = "SELECT ". $prefixo ."cidade.uf, ". $prefixo ."endereco.logradouro, ". $prefixo ."endereco.cep, ". $prefixo ."cidade.cidade, ". $prefixo ."bairro.bairro, ". $prefixo ."endereco.tipo_logradouro,
   ". $prefixo ."endereco.complemento  FROM
   ". $prefixo ."cidade INNER JOIN ". $prefixo ."endereco ON ". $prefixo ."cidade.id_cidade = ". $prefixo ."endereco.id_cidade
   INNER JOIN ". $prefixo ."bairro ON ". $prefixo ."endereco.id_bairro = ". $prefixo ."bairro.id_bairro
   where ". $prefixo ."cidade.cep is null OR ". $prefixo ."cidade.cep = ''
   order by ". $prefixo ."cidade.uf, ". $prefixo ."cidade.cidade, ". $prefixo ."endereco.logradouro";
    ...
    while (!$rs_logr->EOF)
    {
        ...
        $dados[9] = "";   // Longitude
        if (!empty($dados[2])) {
            $comando  = "SELECT latitude, longitude FROM ". $prefixo ."geo WHERE cep = '" . $dados[3] . "'";
            $rs_geo = $Db->Execute($comando);
            if ($rs_geo === false && !$rs_geo->EOF) {
                echo "<br>*** Erro ao acessar tabela ". $prefixo ."geo (logradouros): " . $Db->ErrorMsg();
                exit ;
            }
            if (!$rs_geo->EOF) {
