...
*/
	...
	var $ssh_handle  = false;
	var $ssh_process = false;
	var $ssh_file = "";
...
*/
	...
	*/
    ...
    function startSSH($arr_ssh)
    {
        ...
        if(!empty($arr_ssh) && isset($arr_ssh['ssh_server']) && !empty($arr_ssh['ssh_server']))
        {
            $ret = true;
            ...
            $old_dir = getcwd();
            chdir(__DIR__ . "/ssh/");
            //$this->ssh_file = uniqid(rand(1000000,9999999), true) . '.' . uniqid (rand(1,9999999), true);
            $this->ssh_file = md5(serialize($arr_ssh));

            $str_command  = ".".DIRECTORY_SEPARATOR."scriptcaseTunnel_". $_so ."_". $_64 . DIRECTORY_SEPARATOR."scriptcaseTunnel " . $this->resolveHostIpv4($arr_ssh['ssh_server']) . " ";
            if(!empty($arr_ssh['ssh_port']))
            {
                $str_command .= $arr_ssh['ssh_port'] . " ";
            }
            $str_command .= $arr_ssh['ssh_user'] . " ";
            if(isset($arr_ssh['ssh_pass']) && !empty($arr_ssh['ssh_pass']))
            {
                $str_command .= "--ssh_password " . base64_encode($arr_ssh['ssh_pass']) . " ";
            }
            if(isset($arr_ssh['ssh_privatecert']) && !empty($arr_ssh['ssh_privatecert']))
            {
                $_cert = str_replace("\\", "/", $arr_ssh['ssh_privatecert']);
                if(strpos($_cert, " ")!==false)
                {
                    $_cert = '"' . $_cert . '"';
                }
                $str_command .= "--ssh_key " . $_cert . " ";
            }
            $str_command .= $arr_ssh['ssh_localportforwarding'] . " ". $arr_ssh['ssh_localserver'] . " " . $arr_ssh['ssh_localport'];
            $str_command .= " --output_file " . $this->ssh_file;

            if(strpos($str_command, ';') !== false){
                return false;
            }

            if($_so == 'windows')
            {
                $this->ssh_handle = popen("start /B $str_command", "r");

                exec('powershell -Command "(Get-Process scriptcaseTunnel | Sort-Object StartTime -Descending | Select-Object -First 1).Id"', $output);

                if (!empty($output[0])) {
                    $this->ssh_process = intval($output[0]);
                }
            }
            elseif($_so == 'linux')
            {
                shell_exec($str_command . " > ./tmp/null &");
            }
            else{
                $descriptor = [
                    0 => ["pipe", "r"],
                ...
                $process = proc_open($str_command, $descriptor, $pipes);
                if(is_resource($process))
                {
                    $this->ssh_handle = $process;
                }
            }

            //aguarda criação do arquivo ou timeout
            if(!is_file("tmp/" . $this->ssh_file))
            {
                sleep(1);
                $start_time = time(); //set startup time;
                while(((time() - $start_time) < 10) && !is_file("tmp/" . $this->ssh_file)){
                }
            }
    ...
    function killSSH($arr_ssh)
    {
        if($this->ssh_handle)
        {
            if($this->getSO() != 'windows')
            {
                shell_exec("kill -9 \$(ps aux | grep scriptcaseTunnel | egrep -v 'grep' | awk '{print \$2}')");
            }
            else{
                pclose($this->ssh_handle);
                if($this->ssh_process) {
                    exec("taskkill /PID $this->ssh_process /F");
                }
                $this->ssh_handle = false;
                $this->ssh_process = false;
            }

            if(is_file(__DIR__ . "/ssh/tmp/" . $this->ssh_file))
            {
                unlink(__DIR__ . "/ssh/tmp/" . $this->ssh_file);
            }
            $this->ssh_file = "";
        }
    }
    ...
    function getSSHCommand($arr_ssh)
    {
        ...
        if(isset($arr_ssh['ssh_pass']) && !empty($arr_ssh['ssh_pass']))
        {
            if($_so == 'windows')
            {
                $str_command  = __DIR__ . "/ssh/scriptcaseTunnel " . $arr_ssh['ssh_server'] . " ";
                if(!empty($arr_ssh['ssh_port']))
                {
                    $str_command .= $arr_ssh['ssh_port'] . " ";
                }
                $str_command .= $arr_ssh['ssh_user'] . " --ssh_password " . $arr_ssh['ssh_pass'] . " " . $arr_ssh['ssh_localportforwarding'] . " ". $arr_ssh['ssh_localserver'] . " " . $arr_ssh['ssh_localport'];
            }
        }
        else
        {
    ...
    function resolveHostIpv4($str_host){
        return filter_var($str_host, FILTER_VALIDATE_IP) ? $str_host : gethostbyname(rtrim($str_host, '.'));
    }
