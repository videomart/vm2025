...
class ADODB_ibase extends ADOConnection {
	...
	{  
		...
		$fn = (isset($arrExtraArgs['persist']) && $arrExtraArgs['persist']) ? 'ibase_pconnect':'ibase_connect';

        if(isset($arrExtraArgs['ibase_dialect']) && !empty($arrExtraArgs['ibase_dialect']))
        {
            $this->dialect = $arrExtraArgs['ibase_dialect'];
        }
        if(isset($arrExtraArgs['ibase_rolename']) && !empty($arrExtraArgs['ibase_rolename']))
        {
            $this->role = $arrExtraArgs['ibase_rolename'];
        }
	...
	{
        $arrExtraArgs['persist'] = true;
		return $this->_connect($argHostname, $argUsername, $argPassword, $argDatabasename,$arrExtraArgs, $argDatabaseEncoding);
	}	
