...
function adodb_pdo_type($t, $precision=0)
{

    if($precision>0 && $precision<=4 && $t==2) $t = 4;
    switch($t) {
...
class ADODB_pdo extends ADOConnection {
    ...
    {
        //file_put_contents("C:/wwwroot/teste.txt", "PRE CONECTOU: ". $argDSN ."\r\n-----------------\r\n", FILE_APPEND);
        ...
        if ($argDatabasename) {
            if(strpos($argDatabasename, ' ')!==false) {
                $argDatabasename = "'". $argDatabasename ."'";
            }
            $argDSN .= ';dbname='.$argDatabasename;
        }

        if(strpos($argDSN, "charset")===false) {
        ...
        try {
            ...
            file_put_contents("C:/wwwroot/teste.txt", "CONECTOU: ". $argUsername ."\r\n-----------------\r\n", FILE_APPEND);
            file_put_contents("C:/wwwroot/teste.txt", "CONECTOU: ". $argPassword ."\r\n-----------------\r\n", FILE_APPEND);
            file_put_contents("C:/wwwroot/teste.txt", "CONECTOU: ". print_r($arrExtraArgs, true) ."\r\n-----------------\r\n", FILE_APPEND);
            */
    ...
    {
        if(isset($this->_driver) && !empty($this->_driver) && method_exists($this->_driver, "_insertid"))
        {
            return $this->_driver->_insertid($table, $column, $sequence);
        }
        else
        {
        ...
        if (!$magic_quotes) {
            if ($this->dsnType != 'odbc' && ADODB_PHPVER >= 0x4300) {
                if ($this->_connectionID) {
                    ...
                    return trim($this->_connectionID->quote((string)$s));
                }
            }
...
#                $false = false;
        ...
        elseif (isset($arr['native_type']) && $arr['native_type'] <> "null") {
            ...
            $o->type = adodb_pdo_type($arr['pdo_type'], $arr['precision']);
        }
