   ...
   function gc($iMaxLifeTime) {
    ...
    function conect($perfil, $tab_name, $dir, $dir_prod_conf="")
    {
        ...
        if (isset($prod_ini_xml["PROFILE"]) && is_array($prod_ini_xml["PROFILE"]) && in_array($perfil, array_keys($prod_ini_xml["PROFILE"])))
        {
			...
			$postgres_sslmode = decode_string($prod_ini_xml["PROFILE"][$perfil]["postgres_sslmode"]);
			$postgres_sslrootcert = decode_string($prod_ini_xml["PROFILE"][$perfil]["postgres_sslrootcert"]);
			$postgres_sslkey = decode_string($prod_ini_xml["PROFILE"][$perfil]["postgres_sslkey"]);
			$postgres_sslcert = decode_string($prod_ini_xml["PROFILE"][$perfil]["postgres_sslcert"]);
        }
        else
        {
            return false;
