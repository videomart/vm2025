...
function scEventControl_active(iSeqRow) {
  if (scEventControl_data["titulo_" + iSeqRow] && scEventControl_data["titulo_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["titulo_" + iSeqRow] && scEventControl_data["titulo_" + iSeqRow]["change"]) {
    return true;
  }
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_' + iSeqRow).bind('change', function() { sc_form_markup_custo_id__onchange(this, iSeqRow) });
  $('#id_sc_field_titulo_' + iSeqRow).bind('blur', function() { setTimeout(function() {sc_form_markup_custo_titulo__onblur('#id_sc_field_titulo_' + iSeqRow, iSeqRow);}, 300) })
                                     .bind('change', function() { sc_form_markup_custo_titulo__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_markup_custo_titulo__onfocus(this, iSeqRow) });
  $('#id_sc_field_aliquota_' + iSeqRow).bind('blur', function() { setTimeout(function() {sc_form_markup_custo_aliquota__onblur('#id_sc_field_aliquota_' + iSeqRow, iSeqRow);}, 300) })
                                       .bind('change', function() { sc_form_markup_custo_aliquota__onchange(this, iSeqRow) })
                                       .bind('focus', function() { sc_form_markup_custo_aliquota__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_markup_custo_id__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_form_markup_custo_titulo__onblur(oThis, iSeqRow) {
  do_ajax_form_markup_custo_validate_titulo_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_markup_custo_titulo__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_markup_custo_titulo__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_markup_custo_aliquota__onblur(oThis, iSeqRow) {
  do_ajax_form_markup_custo_validate_aliquota_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_markup_custo_aliquota__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_markup_custo_aliquota__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function scHidePage(pageNo) {
        $("#id_form_markup_custo_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_markup_custo')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
