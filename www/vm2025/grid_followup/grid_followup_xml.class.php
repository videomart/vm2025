...
function actionBar_getStateHide($buttonName)
{
   ...
   function inicializa_vars()
   {
      ...
      if (!is_file($this->Ini->root . $this->Ini->path_link . "grid_followup/grid_followup_res_xml.class.php"))
      {
          $this->Tem_xml_res  = false;
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['embutida'] && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_label']))
      {
          $this->Xml_tag_label = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_label'];
          $this->New_Format    = true;
      }
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_return']);
          if ($this->Tem_xml_res) {
      ...
      $this->Arquivo     .= "_grid_followup";
      $this->Arquivo_view = $this->Arquivo . "_view.xml";
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "grid_followup.xml";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_name'] .= ".xml";
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['embutida'])
         { 
             $this->xml_registro .= "<" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['embutida_tit'] . ">\r\n";
         }
         elseif ($this->New_Format)
         {
             $this->xml_registro = "<grid_followup>\r\n";
         }
         else
         {
             $this->xml_registro = "<grid_followup";
      ...
      { 
          ...
          { 
              ...
              $this->Res = new grid_followup_res_xml();
              $this->prep_modulos("Res");
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_res_grid'] = true;
              $this->Res->monta_xml();
          } 
          if (!$this->Ini->sc_export_ajax) {
          ...
          { 
              ...
              { 
                  $str_zip   = "";
                  $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_res_file']['xml'];
                  ...
                  unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_res_file']['xml']);
              }
              if ($this->Grava_view)
              {
                  $str_zip    = "";
                  ...
                  { 
                      $str_zip   = "";
                      $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_res_file']['view'];
                      $Arq_input = (FALSE !== strpos($Arq_res, ' ')) ? " \"" . $Arq_res . "\"" :  $Arq_res;
                      if (FALSE !== strpos(strtolower(php_uname()), 'windows')) 
                      {
                          $str_zip = "zip.exe " . strtoupper($Parm_pass) . " -j -u " . $this->Xml_password . " " . $Zip_f . " " . $Arq_input;
                      ...
                      unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_res_file']['view']);
                  }
              } 
              else 
              {
              ...
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_res_grid']);
          } 
      }
   ...
   function NM_export_cliente()
   {
         ...
         if ($this->New_Format) {
             $this->cliente .= " " . $this->NM_export_id_proposta(true);
             $this->cliente .= " " . $this->NM_export_relato(true);
         }
         else {
         }
         if ($this->New_Format)
   ...
   function NM_export_data($Grouping=false)
   {
         ...
         {
         if ($Grouping) {
             return " <" . $SC_Label . ">" . $this->trata_dados($this->data) . "</" . $SC_Label . ">\r\n";
         }
         ...
         {
         if ($Grouping) {
             return " " . $SC_Label . " =\"" . $this->trata_dados($this->data) . "\"";
         }
   ...
   function NM_export_data_prevista($Grouping=false)
   {
         ...
         {
             $SC_Label = "data_prevista"; 
         }
         $this->clear_tag($SC_Label); 
         if ($this->New_Format)
         {
         if ($Grouping) {
             return " <" . $SC_Label . ">" . $this->trata_dados($this->data_prevista) . "</" . $SC_Label . ">\r\n";
         }
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->data_prevista) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
         if ($Grouping) {
             return " " . $SC_Label . " =\"" . $this->trata_dados($this->data_prevista) . "\"";
         }
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->data_prevista) . "\"";
         }
   }
   //----- status
   function NM_export_status($Grouping=false)
   {
         ...
         {
         if ($Grouping) {
             return " <" . $SC_Label . ">" . $this->trata_dados($this->status) . "</" . $SC_Label . ">\r\n";
         }
         ...
         {
         if ($Grouping) {
             return " " . $SC_Label . " =\"" . $this->trata_dados($this->status) . "\"";
         }
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->grau_oportunidade))
         {
             $this->grau_oportunidade = sc_convert_encoding($this->grau_oportunidade, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->Xml_tag_label)
         {
         ...
         {
             $SC_Label = "grau_oportunidade"; 
         }
         $this->clear_tag($SC_Label); 
         if ($this->New_Format)
         {
         if ($Grouping) {
             return " <" . $SC_Label . ">" . $this->trata_dados($this->grau_oportunidade) . "</" . $SC_Label . ">\r\n";
         }
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->grau_oportunidade) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
         if ($Grouping) {
             return " " . $SC_Label . " =\"" . $this->trata_dados($this->grau_oportunidade) . "\"";
         }
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->grau_oportunidade) . "\"";
         }
   }
   //----- id
   function NM_export_id($Grouping=false)
   {
         ...
         {
         if ($Grouping) {
             return " <" . $SC_Label . ">" . $this->trata_dados($this->id) . "</" . $SC_Label . ">\r\n";
         }
         ...
         {
         if ($Grouping) {
             return " " . $SC_Label . " =\"" . $this->trata_dados($this->id) . "\"";
         }
   ...
   function NM_export_id_proposta($Grouping=false)
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->look_id_proposta))
         {
             $this->look_id_proposta = sc_convert_encoding($this->look_id_proposta, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->Xml_tag_label)
         {
         ...
         {
             $SC_Label = "id_proposta"; 
         }
         $this->clear_tag($SC_Label); 
         if ($this->New_Format)
         {
         if ($Grouping) {
             return " <" . $SC_Label . ">" . $this->trata_dados($this->look_id_proposta) . "</" . $SC_Label . ">\r\n";
         }
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->look_id_proposta) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
         if ($Grouping) {
             return " " . $SC_Label . " =\"" . $this->trata_dados($this->look_id_proposta) . "\"";
         }
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->look_id_proposta) . "\"";
         }
   }
   //----- relato
   function NM_export_relato($Grouping=false)
   {
         ...
         {
             $this->relato = sc_convert_encoding($this->relato, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->Xml_tag_label)
         {
         ...
         {
         if ($Grouping) {
             return " <" . $SC_Label . ">" . $this->trata_dados($this->relato) . "</" . $SC_Label . ">\r\n";
         }
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->relato) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
         if ($Grouping) {
             return " " . $SC_Label . " =\"" . $this->trata_dados($this->relato) . "\"";
         }
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->relato) . "\"";
         }
   }
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_title'] ?> followup :: XML</TITLE>
...
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['xml_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
