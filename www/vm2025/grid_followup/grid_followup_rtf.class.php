...
function actionBar_getStateHide($buttonName)
{
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo   .= ".rtf";
      $this->Tit_doc    = "grid_followup.rtf";
   }
   //---- 
   ...
   function gera_texto_tag()
   {
     global $nm_lang;
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['rtf_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['rtf_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['rtf_name'] .= ".rtf";
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['rtf_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['rtf_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['rtf_name']);
      }
   ...
   function NM_export_cliente()
   {
         ...
         $this->cliente .= $SC_Lab . ": " . $this->NM_export_id_proposta(true);
         $SC_Lab = (isset($this->New_label['relato'])) ? $this->New_label['relato'] : "RELATO";
         $this->cliente .= (empty($this->cliente)) ? "" : " || ";
         $this->cliente .= $SC_Lab . ": " . $this->NM_export_relato(true);
   ...
   function NM_export_data_prevista($Grouping=false)
   {
         ...
         $this->data_prevista = NM_charset_to_utf8($this->data_prevista);
         $this->data_prevista = str_replace('<', '&lt;', $this->data_prevista);
         $this->data_prevista = str_replace('>', '&gt;', $this->data_prevista);
         if ($Grouping) {
             return $this->data_prevista;
         }
         else {
             $this->Texto_tag .= "<td>" . $this->data_prevista . "</td>\r\n";
         }
   }
   //----- status
   ...
   function NM_export_grau_oportunidade($Grouping=false)
   {
         ...
         $this->grau_oportunidade = str_replace('<', '&lt;', $this->grau_oportunidade);
         $this->grau_oportunidade = str_replace('>', '&gt;', $this->grau_oportunidade);
         if ($Grouping) {
             return $this->grau_oportunidade;
         }
         else {
             $this->Texto_tag .= "<td>" . $this->grau_oportunidade . "</td>\r\n";
         }
   }
   //----- id
   ...
   function NM_export_id_proposta($Grouping=false)
   {
         ...
         $this->look_id_proposta = str_replace('<', '&lt;', $this->look_id_proposta);
         $this->look_id_proposta = str_replace('>', '&gt;', $this->look_id_proposta);
         if ($Grouping) {
             return $this->look_id_proposta;
         }
         else {
             $this->Texto_tag .= "<td>" . $this->look_id_proposta . "</td>\r\n";
         }
   }
   //----- relato
   function NM_export_relato($Grouping=false)
   {
         ...
         $this->relato = str_replace('<', '&lt;', $this->relato);
         $this->relato = str_replace('>', '&gt;', $this->relato);
         if ($Grouping) {
             return $this->relato;
         }
         else {
             $this->Texto_tag .= "<td>" . $this->relato . "</td>\r\n";
         }
   }
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['rtf_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_followup']['rtf_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_title'] ?> followup :: RTF</TITLE>
