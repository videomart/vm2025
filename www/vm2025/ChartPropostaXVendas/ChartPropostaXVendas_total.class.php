...
class ChartPropostaXVendas_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['ChartPropostaXVendas']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['ChartPropostaXVendas']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false, $ind_compara="", $tipo="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("pt_br");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('natureza' => "natureza");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
          $cmps_quebra_atual[] = $cmp;
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['SC_force_fiels_gb'];
      }
      $Save_Format = '';
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['SC_clear_total']);
      $ind_cmps  = 3;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export, $Save_Format)
   {
     ...
     $cmd_simp = "select count(*), count(data), count(distinct data)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
     $comando  = "select count(*), count(SC_metric1), count(distinct SC_metric1)#@#cmps_quebras#@# from (";
     $comando .= "select data as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
     $comando .= "select " . $cmps_quebras2 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['natureza']['reg'] = "S";
      $format_dimensions['natureza']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['tot_geral'] = array() ;  
      $nm_comando = "select count(*), count(data), count(distinct data) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['where_pesq']; 
      ...
      if ((isset($this->Ini->nm_bases_vfp) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_vfp)) || (isset($this->Ini->nm_bases_odbc) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_odbc)))
      {
          $vl_statistic = $this->Calc_statist_manual_sc_free_group_by($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['where_pesq']);
          foreach ($vl_statistic as $ind => $val)
          {
              $rt->fields[$ind] = $val;
          }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['contr_total_geral'] = "OK";
   } 


   function Calc_statist_manual_sc_free_group_by($sql_where="")
   {
       $comando  = "select data from " . $this->Ini->nm_tabela . "  " .  $sql_where;
       $_SESSION['scriptcase']['sc_sql_ult_comando'] = $comando;
       $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
   ...
   //-----  natureza
   function quebra_natureza_sc_free_group_by($natureza, $Where_qb, $Cmp_Name) 
   {
      ...
      $nm_comando = "select count(*), count(data), count(distinct data) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['where_pesq']; 
      $nm_comando = $this->Ajust_statistic($nm_comando);
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['where_pesq'])) 
      { 
      ...
      { 
         $where_ok = $_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['where_pesq'] . " and " . $Where_qb ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['ChartPropostaXVendas']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_natureza[0] = NM_encode_input(sc_strip_script($natureza));
      }
      else {
          $tot_natureza[0] = sc_strip_script($natureza) ; 
      }
      $tot_natureza[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_natureza[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_natureza[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_natureza;
   } 
