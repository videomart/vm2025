...
$old_error_handler = set_error_handler("form_categoria_scriptcase_error_handler");

function form_categoria_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_categoria']['contr_erro']) && $_SESSION['scriptcase']['form_categoria']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_categoria->contr_form_categoria->Apl_com_erro = true;
            if ($inicial_form_categoria->contr_form_categoria->NM_ajax_flag)
            {
                $inicial_form_categoria->contr_form_categoria->NM_ajax_info['errList']['geral_form_categoria'][] = $msg;
                return;
            }
            form_categoria_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_categoria_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_categoria'])  && '' != $_SESSION['scriptcase']['error_icon']['form_categoria'])  ? $_SESSION['scriptcase']['error_icon']['form_categoria']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_categoria']) && '' != $_SESSION['scriptcase']['error_close']['form_categoria']) ? $_SESSION['scriptcase']['error_close']['form_categoria'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_categoria']) && '' != $_SESSION['scriptcase']['error_span_title']['form_categoria']) ? $_SESSION['scriptcase']['error_span_title']['form_categoria'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_categoria']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_categoria']) ? $_SESSION['scriptcase']['error_icon_title']['form_categoria'] : '';
?>
