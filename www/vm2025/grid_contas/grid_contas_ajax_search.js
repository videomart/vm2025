...
Table_sv_fil[0] = "moeda";
Table_sv_fil[1] = "id_empresa";
Table_sv_fil[2] = "data";
Table_sv_fil[3] = "referencia";
     ...
     .done(function(json_sel_fil) {
        ...
        if (oResp["exec_JS"]) {
          for (i = 0; i < oResp["exec_JS"].length; i++) {
               eval ('if ("function" == typeOf ' + oResp["exec_JS"][i]["function"] + ')' + oResp["exec_JS"][i]["function"] + '("' + oResp["exec_JS"][i]["parm"] + '");');
          }
        }
        if ($("#SC_data_cond") && search_get_sel_txt("SC_data_cond").substring(0, 3) == "bi_") {
            formata_bi_data(search_get_sel_txt("SC_data_cond"), SC_data_bi_opc, SC_data_bi_dt);
        }
