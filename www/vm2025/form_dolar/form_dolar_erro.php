...
$old_error_handler = set_error_handler("form_dolar_scriptcase_error_handler");

function form_dolar_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_dolar']['contr_erro']) && $_SESSION['scriptcase']['form_dolar']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_dolar->contr_form_dolar->Apl_com_erro = true;
            if ($inicial_form_dolar->contr_form_dolar->NM_ajax_flag)
            {
                $inicial_form_dolar->contr_form_dolar->NM_ajax_info['errList']['geral_form_dolar'][] = $msg;
                return;
            }
            form_dolar_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_dolar_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_dolar'])  && '' != $_SESSION['scriptcase']['error_icon']['form_dolar'])  ? $_SESSION['scriptcase']['error_icon']['form_dolar']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_dolar']) && '' != $_SESSION['scriptcase']['error_close']['form_dolar']) ? $_SESSION['scriptcase']['error_close']['form_dolar'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_dolar']) && '' != $_SESSION['scriptcase']['error_span_title']['form_dolar']) ? $_SESSION['scriptcase']['error_span_title']['form_dolar'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_dolar']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_dolar']) ? $_SESSION['scriptcase']['error_icon_title']['form_dolar'] : '';
?>
