...
class chart_leads_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_leads']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_leads']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false, $ind_compara="", $tipo="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("pt_br");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('assunto' => "ASSUNTO");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
          $cmps_quebra_atual[] = $cmp;
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['SC_force_fiels_gb'];
      }
      $Save_Format = '';
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['SC_clear_total']);
      $ind_cmps  = 2;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export, $Save_Format)
   {
     ...
     $cmd_simp = "select count(*), count(ID)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
     $comando  = "select count(*), count(SC_metric1)#@#cmps_quebras#@# from (";
     $comando .= "select ID as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
     $comando .= "select " . $cmps_quebras2 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $nm_comando = $comando;
      $tmp_date_1_data = $this->nm_data->CalculaData(date('dmY') , 'ddmmaaaa', '-', 29, 0, 0);
      $tmp_date_2_data = date('Y-m-d');
      $nm_comando = str_replace('{SC_TMP_DATE_1_data}', $tmp_date_1_data, $nm_comando);
      $nm_comando = str_replace('{SC_TMP_DATE_2_data}', $tmp_date_2_data, $nm_comando);
      $comando = $nm_comando;
      ...
      $format_dimensions['assunto']['reg'] = "S";
      $format_dimensions['assunto']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['tot_geral'] = array() ;  
      $nm_comando = "select count(*), count(ID) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['where_pesq']; 
      $tmp_date_1_data = $this->nm_data->CalculaData(date('dmY') , 'ddmmaaaa', '-', 29, 0, 0);
      $tmp_date_2_data = date('Y-m-d');
      $nm_comando = str_replace('{SC_TMP_DATE_1_data}', $tmp_date_1_data, $nm_comando);
      $nm_comando = str_replace('{SC_TMP_DATE_2_data}', $tmp_date_2_data, $nm_comando);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['contr_total_geral'] = "OK";
   } 

   //-----  assunto
   function quebra_assunto_sc_free_group_by($assunto, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_assunto = array() ;  
      $nm_comando = "select count(*), count(ID) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['where_pesq'])) 
      { 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_leads']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_assunto[0] = NM_encode_input(sc_strip_script($assunto));
      }
      else {
          $tot_assunto[0] = sc_strip_script($assunto) ; 
      }
      $tot_assunto[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_assunto[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_assunto;
   } 
