...
$old_error_handler = set_error_handler("form_price_scriptcase_error_handler");

function form_price_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_price']['contr_erro']) && $_SESSION['scriptcase']['form_price']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_price->contr_form_price->Apl_com_erro = true;
            if ($inicial_form_price->contr_form_price->NM_ajax_flag)
            {
                $inicial_form_price->contr_form_price->NM_ajax_info['errList']['geral_form_price'][] = $msg;
                return;
            }
            form_price_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_price_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_price'])  && '' != $_SESSION['scriptcase']['error_icon']['form_price'])  ? $_SESSION['scriptcase']['error_icon']['form_price']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_price']) && '' != $_SESSION['scriptcase']['error_close']['form_price']) ? $_SESSION['scriptcase']['error_close']['form_price'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_price']) && '' != $_SESSION['scriptcase']['error_span_title']['form_price']) ? $_SESSION['scriptcase']['error_span_title']['form_price'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_price']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_price']) ? $_SESSION['scriptcase']['error_icon_title']['form_price'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_price']['error_buffer'] .= ob_get_flush();
}
