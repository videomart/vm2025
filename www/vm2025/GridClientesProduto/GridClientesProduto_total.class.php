...
class GridClientesProduto_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['GridClientesProduto']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['GridClientesProduto']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false, $ind_compara="", $tipo="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("pt_br");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('empresa_email' => "empresa.EMAIL",'itemproposta_modelo' => "itemproposta.MODELO",'marca_marca' => "marca.MARCA");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
          $cmps_quebra_atual[] = $cmp;
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['SC_force_fiels_gb'];
      }
      $Save_Format = '';
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['SC_clear_total']);
      $ind_cmps  = 4;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export, $Save_Format)
   {
      ...
      $cmp_sql_tp_num = array('proposta_id' => 'N');
     $cmd_simp = "select count(*), count(empresa.EMAIL), count(itemproposta.MODELO), count(marca.MARCA)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
     $comando  = "select count(*), count(SC_metric1), count(SC_metric2), count(SC_metric3)#@#cmps_quebras#@# from (";
     $comando .= "select empresa.EMAIL as SC_metric1,itemproposta.MODELO as SC_metric2,marca.MARCA as SC_metric3, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
     $comando .= "select " . $cmps_quebras2 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['empresa_email']['reg'] = "S";
      $format_dimensions['empresa_email']['msk'] = "";
      $format_dimensions['itemproposta_modelo']['reg'] = "S";
      $format_dimensions['itemproposta_modelo']['msk'] = "";
      $format_dimensions['marca_marca']['reg'] = "S";
      $format_dimensions['marca_marca']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "empresa_email") {
              }
              if ($Cada_dim == "itemproposta_modelo") {
              }
              if ($Cada_dim == "marca_marca") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['tot_geral'] = array() ;  
      $nm_comando = "select count(*), count(empresa.EMAIL), count(itemproposta.MODELO), count(marca.MARCA) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['tot_geral'][4] = $rt->fields[3]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['contr_total_geral'] = "OK";
   } 

   //-----  empresa_email
   function quebra_empresa_email_sc_free_group_by($empresa_email, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_empresa_email = array() ;  
      $nm_comando = "select count(*), count(empresa.EMAIL), count(itemproposta.MODELO), count(marca.MARCA) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['where_pesq']; 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_empresa_email[0] = NM_encode_input(sc_strip_script($empresa_email));
      }
      else {
          $tot_empresa_email[0] = sc_strip_script($empresa_email) ; 
      }
      $tot_empresa_email[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_empresa_email[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_empresa_email[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_empresa_email[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_empresa_email;
   } 

   //-----  itemproposta_modelo
   function quebra_itemproposta_modelo_sc_free_group_by($itemproposta_modelo, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_itemproposta_modelo = array() ;  
      $nm_comando = "select count(*), count(empresa.EMAIL), count(itemproposta.MODELO), count(marca.MARCA) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['where_pesq'])) 
      { 
         $nm_comando .= " where " . $Where_qb ; 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_itemproposta_modelo[0] = NM_encode_input(sc_strip_script($itemproposta_modelo));
      }
      else {
          $tot_itemproposta_modelo[0] = sc_strip_script($itemproposta_modelo) ; 
      }
      $tot_itemproposta_modelo[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_itemproposta_modelo[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_itemproposta_modelo[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_itemproposta_modelo[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_itemproposta_modelo;
   } 

   //-----  marca_marca
   function quebra_marca_marca_sc_free_group_by($marca_marca, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_marca_marca = array() ;  
      $nm_comando = "select count(*), count(empresa.EMAIL), count(itemproposta.MODELO), count(marca.MARCA) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['where_pesq'])) 
      { 
         $nm_comando .= " where " . $Where_qb ; 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['GridClientesProduto']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_marca_marca[0] = NM_encode_input(sc_strip_script($marca_marca));
      }
      else {
          $tot_marca_marca[0] = sc_strip_script($marca_marca) ; 
      }
      $tot_marca_marca[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_marca_marca[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_marca_marca[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_marca_marca[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_marca_marca;
   } 
