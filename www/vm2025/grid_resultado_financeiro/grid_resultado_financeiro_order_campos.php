...
<?php
   ...
   $Ord_Cmp = new grid_resultado_financeiro_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_resultado_financeiro_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select_orig'] = array();
   }
   ...
   $tab_labels["moeda"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['labels']["moeda"])) ? $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['labels']["moeda"] : "MOEDA";
   $tab_ger_campos['creditos'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['creditos'] = "creditos";
   }
   else
   {
       $tab_def_campos['creditos'] = "SUM(CREDITO)";
   }
   $tab_labels["creditos"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['labels']["creditos"])) ? $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['labels']["creditos"] : "Creditos";
   $tab_ger_campos['debitos'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['debitos'] = "debitos";
   }
   else
   {
       $tab_def_campos['debitos'] = "SUM(DEBITO)";
   }
   $tab_labels["debitos"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['labels']["debitos"])) ? $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['labels']["debitos"] : "Debitos";
   ...
   {
       $tab_def_campos['subtotal'] = "SUM(CREDITO - DEBITO)";
   }
   $tab_labels["subtotal"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['labels']["subtotal"])) ? $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['labels']["subtotal"] : "Subtotais";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_resultado_financeiro_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_resultado_financeiro']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
