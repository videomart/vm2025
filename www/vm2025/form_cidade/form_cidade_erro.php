...
$old_error_handler = set_error_handler("form_cidade_scriptcase_error_handler");

function form_cidade_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_cidade']['contr_erro']) && $_SESSION['scriptcase']['form_cidade']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_cidade->contr_form_cidade->Apl_com_erro = true;
            if ($inicial_form_cidade->contr_form_cidade->NM_ajax_flag)
            {
                $inicial_form_cidade->contr_form_cidade->NM_ajax_info['errList']['geral_form_cidade'][] = $msg;
                return;
            }
            form_cidade_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_cidade_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_cidade'])  && '' != $_SESSION['scriptcase']['error_icon']['form_cidade'])  ? $_SESSION['scriptcase']['error_icon']['form_cidade']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_cidade']) && '' != $_SESSION['scriptcase']['error_close']['form_cidade']) ? $_SESSION['scriptcase']['error_close']['form_cidade'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_cidade']) && '' != $_SESSION['scriptcase']['error_span_title']['form_cidade']) ? $_SESSION['scriptcase']['error_span_title']['form_cidade'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_cidade']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_cidade']) ? $_SESSION['scriptcase']['error_icon_title']['form_cidade'] : '';
?>
