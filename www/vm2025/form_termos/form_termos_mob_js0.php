...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_termos_mob']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_termos_mob']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_termos_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
    NovaJanela = window.open ("form_termos_mob_help.php?nm_cod_campo=" + campo + "&language=<?php echo $this->Ini->path_lang . $this->Ini->str_lang?>&help_css=<?php echo '../_lib/css/' . $this->Ini->str_schema_all . '_form.css'; ?>", "ScriptCase", "height=100, width=300, resizable, scrollbars");
}
