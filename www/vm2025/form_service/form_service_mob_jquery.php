...
function scFocusField(sField) {
  ...
  if ($oField.length > 0) {
    switch ($oField[0].name) {
      ...
      case 'descricao':
      case 'sintoma':
        sc_exib_ocult_pag('form_service_mob_form0');
        break;
      case 'dataorc':
      case 'maoobra':
      case 'material':
      ...
      case 'servico':
      case 'mat':
        sc_exib_ocult_pag('form_service_mob_form1');
        break;
    }
  }
...
function displayChange_field_servico(row, status) {
    var fieldId;
        if ("on" == status && typeof $("#nmsc_iframe_liga_form_work_mob")[0].contentWindow.scRecreateSelect2 === "function") {
                $("#nmsc_iframe_liga_form_work_mob")[0].contentWindow.scRecreateSelect2();
        }
        $("#nmsc_iframe_liga_form_work_mob")[0].contentWindow.specificStyle();
}

function displayChange_field_mat(row, status) {
    var fieldId;
        if ("on" == status && typeof $("#nmsc_iframe_liga_form_material_mob")[0].contentWindow.scRecreateSelect2 === "function") {
                $("#nmsc_iframe_liga_form_material_mob")[0].contentWindow.scRecreateSelect2();
        }
        $("#nmsc_iframe_liga_form_material_mob")[0].contentWindow.specificStyle();
}
...
function scHidePage(pageNo) {
        $("#id_form_service_mob_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_service_mob')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
