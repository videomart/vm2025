...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_licenca_mob']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_licenca_mob']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_licenca_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
    window.open('<?php echo $this->Ini->path_link . "" . SC_dir_app_name('form_licenca_mob') . "/form_licenca_mob_iframe_prt.php?path_botoes=" . $this->Ini->path_botoes . "&script_case_init=" . $this->form_encode_input($this->Ini->sc_page);?>"&apl_prt=form_licenca_mob.php&opcao=print&cor_print=AM','','location=no,menubar,resizable,scrollbars,status=no,toolbar');
}
