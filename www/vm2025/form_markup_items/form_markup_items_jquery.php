...
function scEventControl_init(iSeqRow) {
  scEventControl_data["id_custo_markup_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["aliquota_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["id_markup_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["id_custo_markup_" + iSeqRow] && scEventControl_data["id_custo_markup_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_custo_markup_" + iSeqRow] && scEventControl_data["id_custo_markup_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["aliquota_" + iSeqRow] && scEventControl_data["aliquota_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["aliquota_" + iSeqRow] && scEventControl_data["aliquota_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["id_markup_" + iSeqRow] && scEventControl_data["id_markup_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_markup_" + iSeqRow] && scEventControl_data["id_markup_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scEventControl_onFocus(oField, iSeq) {
  ...
  if ("id_custo_markup_" + iSeq == fieldName) {
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_' + iSeqRow).bind('change', function() { sc_form_markup_items_id__onchange(this, iSeqRow) });
  $('#id_sc_field_id_markup_' + iSeqRow).bind('blur', function() { setTimeout(function() {sc_form_markup_items_id_markup__onblur('#id_sc_field_id_markup_' + iSeqRow, iSeqRow);}, 300) })
                                        .bind('change', function() { sc_form_markup_items_id_markup__onchange(this, iSeqRow) })
                                        .bind('focus', function() { sc_form_markup_items_id_markup__onfocus(this, iSeqRow) });
  $('#id_sc_field_id_custo_markup_' + iSeqRow).bind('blur', function() { setTimeout(function() {sc_form_markup_items_id_custo_markup__onblur('#id_sc_field_id_custo_markup_' + iSeqRow, iSeqRow);}, 300) })
                                              .bind('change', function() { sc_form_markup_items_id_custo_markup__onchange(this, iSeqRow) })
                                              .bind('focus', function() { sc_form_markup_items_id_custo_markup__onfocus(this, iSeqRow) });
  $('#id_sc_field_aliquota_' + iSeqRow).bind('blur', function() { setTimeout(function() {sc_form_markup_items_aliquota__onblur('#id_sc_field_aliquota_' + iSeqRow, iSeqRow);}, 300) })
                                       .bind('change', function() { sc_form_markup_items_aliquota__onchange(this, iSeqRow) })
                                       .bind('focus', function() { sc_form_markup_items_aliquota__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_markup_items_id__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_form_markup_items_id_markup__onblur(oThis, iSeqRow) {
  do_ajax_form_markup_items_validate_id_markup_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_markup_items_id_markup__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_markup_items_id_markup__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_markup_items_id_custo_markup__onblur(oThis, iSeqRow) {
  do_ajax_form_markup_items_validate_id_custo_markup_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_markup_items_id_custo_markup__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  do_ajax_form_markup_items_event_id_custo_markup__onchange(iSeqRow);
  nm_check_insert(iSeqRow);
}

function sc_form_markup_items_id_custo_markup__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_markup_items_aliquota__onblur(oThis, iSeqRow) {
  do_ajax_form_markup_items_validate_aliquota_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_markup_items_aliquota__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_markup_items_aliquota__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
        displayChange_field("id_custo_markup_", "", status);
        displayChange_field("aliquota_", "", status);
        displayChange_field("id_markup_", "", status);
}

function displayChange_row(row, status) {
        displayChange_field_id_custo_markup_(row, status);
        displayChange_field_aliquota_(row, status);
        displayChange_field_id_markup_(row, status);
}

function displayChange_field(field, row, status) {
        if ("id_custo_markup_" == field) {
                displayChange_field_id_custo_markup_(row, status);
        }
        if ("aliquota_" == field) {
                displayChange_field_aliquota_(row, status);
...
function displayChange_field_id_custo_markup_(row, status) {
    var fieldId;
        if ("on" == status) {
                if ("all" == row) {
                        var fieldList = $(".css_id_custo_markup___obj");
                        for (var i = 0; i < fieldList.length; i++) {
                                $($(fieldList[i]).attr("id")).select2("destroy");
                        }
                }
                else {
                        $("#id_sc_field_id_custo_markup_" + row).select2("destroy");
                }
                scJQSelect2Add(row, "id_custo_markup_");
        }
}

function displayChange_field_aliquota_(row, status) {
    var fieldId;
}

function displayChange_field_id_markup_(row, status) {
    var fieldId;
}

function scRecreateSelect2() {
        displayChange_field_id_custo_markup_("all", "on");
}
...
function scHidePage(pageNo) {
        $("#id_form_markup_items_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_markup_items')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
...
function scJQSelect2Add(seqRow, specificField) {
  if (null == specificField || "id_custo_markup_" == specificField) {
    scJQSelect2Add_id_custo_markup_(seqRow);
  }
} // scJQSelect2Add

function scJQSelect2Add_id_custo_markup_(seqRow) {
  var elemSelector = "all" == seqRow ? ".css_id_custo_markup__obj" : "#id_sc_field_id_custo_markup_" + seqRow;
  $(elemSelector).select2(
    {
      containerCssClass: 'css_id_custo_markup__obj',
      dropdownCssClass: 'css_id_custo_markup__obj',
      language: {
        noResults: function() {
  ...
  $('#select2-id_sc_field_id_custo_markup_' + seqRow + '-container').addClass('css_id_custo_markup__obj');
} // scJQSelect2Add
...
function scJQElementsAdd(iLine) {
  ...
  setTimeout(function () { if ('function' == typeof displayChange_field_id_custo_markup_) { displayChange_field_id_custo_markup_(iLine, "on"); } }, 150);
} // scJQElementsAdd
