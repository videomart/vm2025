...
function scEventControl_onCalculator_aliquota() {
  if (!_scCalculatorControl["id_sc_field_aliquota"]) {
    ...
    do_ajax_form_markup_event_aliquota_onblur();
  }
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id' + iSeqRow).bind('change', function() { sc_form_markup_id_onchange(this, iSeqRow) });
  $('#id_sc_field_titulo' + iSeqRow).bind('blur', function() { setTimeout(function() {sc_form_markup_titulo_onblur('#id_sc_field_titulo' + iSeqRow, iSeqRow);}, 300) })
                                    .bind('change', function() { sc_form_markup_titulo_onchange(this, iSeqRow) })
  ...
  $('#id_sc_field_markup' + iSeqRow).bind('blur', function() { setTimeout(function() {sc_form_markup_markup_onblur('#id_sc_field_markup' + iSeqRow, iSeqRow);}, 300) })
                                    .bind('change', function() { sc_form_markup_markup_onchange(this, iSeqRow) })
  ...
  $('#id_sc_field_markupitems' + iSeqRow).bind('blur', function() { sc_form_markup_markupitems_onblur('#id_sc_field_markupitems' + iSeqRow, iSeqRow) })
                                         .bind('change', function() { sc_form_markup_markupitems_onchange(this, iSeqRow) })
                                         .bind('focus', function() { sc_form_markup_markupitems_onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_markup_id_onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}
...
function sc_form_markup_titulo_onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}
...
function sc_form_markup_aliquota_onchange(oThis, iSeqRow) {
  scJQSlideValue("aliquota" + iSeqRow, iSeqRow);
  scMarkFormAsChanged();
...
function sc_form_markup_markup_onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}
...
function sc_form_markup_markupitems_onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}
...
function displayChange_field_markupitems(row, status) {
    var fieldId;
        if ("on" == status && typeof $("#nmsc_iframe_liga_form_markup_items")[0].contentWindow.scRecreateSelect2 === "function") {
                $("#nmsc_iframe_liga_form_markup_items")[0].contentWindow.scRecreateSelect2();
        }
}
...
function scHidePage(pageNo) {
        $("#id_form_markup_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_markup')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
