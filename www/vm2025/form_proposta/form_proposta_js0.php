...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_proposta']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_proposta']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_proposta"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_sc_btn_0_ordem = "<?php echo $this->form_encode_input($this->nmgp_dados_form['id']); ?>";
var var_btn_sc_btn_0_email = "<?php echo $this->form_encode_input($this->nmgp_dados_form['email']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_consulta_fatura_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "ordem*scin' + var_btn_sc_btn_0_ordem + '*scoutemail*scin' + var_btn_sc_btn_0_email + '*scoutNMSC_inicial*scininicio*scoutNMSC_paginacao*scinFULL*scoutlink_remove_margin*scinno*scoutlink_remove_border*scinno*scoutlink_remove_background*scinno*scout"; ?>', 'inicio', '_self', '0', '0', 'consulta_fatura');
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_proposta']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("cod_vend=disabled", "I");
...
{
    document.getElementById('form_proposta_form0').style.width='1px';
    document.getElementById('form_proposta_form0').style.height='0px';
    document.getElementById('form_proposta_form0').style.display='none';
    document.getElementById('form_proposta_form0').style.overflow='scroll';
    document.getElementById('form_proposta_form1').style.width='1px';
    document.getElementById('form_proposta_form1').style.height='0px';
    document.getElementById('form_proposta_form1').style.display='none';
    document.getElementById('form_proposta_form1').style.overflow='scroll';
<?php
    if (!$_SESSION['sc_session'][$this->Ini->sc_page]['form_proposta']['pdf_view']) {
?>
...
?>
   ...
   if ("form_proposta_form0" == N_pagina) {
...
function scCssBlur(oHtmlObj)
{
 ...
 function nm_field_disabled(Fields, Opt) {
  ...
  for (i=0; i < Field.length; i++)
  {
     ...
     if (F_name == "cod_vend")
     {
        $('select[name="cod_vend"]').prop("disabled", F_opc);
        if (F_opc == "disabled" || F_opc == true) {
            $('select[name="cod_vend"]').addClass("scFormInputDisabled");
        }
        else {
            $('select[name="cod_vend"]').removeClass("scFormInputDisabled");
        }
     }
  }
 } // nm_field_disabled
