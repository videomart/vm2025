...
class grid_empresa_duplicadas_pesq
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_empresa_duplicadas_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_empresa_duplicadas_pack_protect_string($Tipo_filter[0]) . "\">.." . htmlspecialchars(grid_empresa_duplicadas_pack_protect_string($Cada_filter) , ENT_QUOTES, 'UTF-8').  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == "ajax_filter_delete")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              $Opt_filter .= "<option value=\"" . grid_empresa_duplicadas_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_empresa_duplicadas_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_cnpj_cpf')
      {
          $cnpj_cpf = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_cnpj_cpf($cnpj_cpf);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_cnpj_cpf($cnpj_cpf)
   {
      $cnpj_cpf = substr($this->Db->qstr($cnpj_cpf), 1, -1);
            $cnpj_cpf_look = (is_string($cnpj_cpf) ? substr($this->Db->qstr($cnpj_cpf), 1, -1) : $cnpj_cpf); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct CNPJ_CPF from (SELECT E.ID, E.EMPRESA,E.CONTATO, E.CNPJ_CPF, E.DATA FROM empresa E INNER JOIN ( SELECT CNPJ_CPF, count(CNPJ_CPF) As Duplicado FROM empresa GROUP BY CNPJ_CPF HAVING count(CNPJ_CPF ) >1) TEMP on E.CNPJ_CPF = TEMP.CNPJ_CPF  ) nm_sel_esp where  #lowerI#CNPJ_CPF#lowerF# like #lowerI#'%" . $cnpj_cpf . "%'#lowerF#"; 
   ...
   function monta_condicao($nome, $condicao, $campo, $campo2 = "", $nome_campo="", $tp_campo="", $tp_unaccent=false)
   {
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['SC_sep_date']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['SC_sep_date']))
          {
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_equl'] . " <span class='scGridFilterDynValue'>" . nm_encode_input($this->cmp_formatado[$nome_campo]) . "</span>##*@@";
            break;
            case "II":     // 
               $op_all = " like ";
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_strt'] . " <span class='scGridFilterDynValue'>" . nm_encode_input($this->cmp_formatado[$nome_campo]) . "</span>##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . ":</span> " . str_replace(array("##ini##","##fim##"), array("<span class='scGridFilterDynValue'>","</span>"),$NM_cond) . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $Nm_accent['cmp_i'] . $nome_sum . $Nm_accent['cmp_f'] . $nm_fim_lower . $Nm_accent['cmp_apos'] . $op_all . $nm_ini_lower . "'%" . $Nm_accent['arg_i'] . sc_sql_escape($this->Ini->nm_tpbanco, $campo) . $Nm_accent['arg_f'] . "%'" . $nm_fim_lower . $Nm_accent['arg_apos'] . $_SESSION['sc_session']['sc_sql_escape'];
                   $this->comando_filtro .= $nm_ini_lower . $Nm_accent['cmp_i'] . $nome . $Nm_accent['cmp_f'] . $nm_fim_lower . $Nm_accent['cmp_apos'] . $op_all . $nm_ini_lower . "'%" . $Nm_accent['arg_i'] . sc_sql_escape($this->Ini->nm_tpbanco, $campo) . $Nm_accent['arg_f'] . "%'" . $nm_fim_lower . $Nm_accent['arg_apos'] . $_SESSION['sc_session']['sc_sql_escape'];
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $lang_like . " <span class='scGridFilterDynValue'>" . nm_encode_input($this->cmp_formatado[$nome_campo]) . "</span>##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_diff'] . " <span class='scGridFilterDynValue'>" . nm_encode_input($this->cmp_formatado[$nome_campo]) . "</span>##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_grtr'] . " <span class='scGridFilterDynValue'>" . nm_encode_input($this->cmp_formatado[$nome_campo]) . "</span>##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " <span class='scGridFilterDynValue'>" . nm_encode_input($this->cmp_formatado[$nome_campo]) . "</span>##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_less'] . " <span class='scGridFilterDynValue'>" . nm_encode_input($this->cmp_formatado[$nome_campo]) . "</span>##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " <span class='scGridFilterDynValue'>" . nm_encode_input($this->cmp_formatado[$nome_campo]) . "</span>##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_betw'] . " <span class='scGridFilterDynValue'>" . $this->cmp_formatado[$nome_campo] . " </span>" . $this->Ini->Nm_lang['lang_srch_and_cond'] . " <span class='scGridFilterDynValue'>" . $this->cmp_formatado[$nome_campo . "_input_2"] . "</span>##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_equl'] . " <span class='scGridFilterDynValue'>" . $nm_cond . "</span>##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= "<span class='scGridFilterDynField'>" . $nmgp_tab_label[$nome_campo] . "</span> " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_empresa_duplicadas/grid_empresa_duplicadas_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
<?php
$vertical_center = '';
?>
...
<?php echo $this->Ini->Ajax_result_set ?>
 ...
 <script type="text/javascript" src="grid_empresa_duplicadas_ajax_search.js"></script>
 <script type="text/javascript" src="grid_empresa_duplicadas_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_empresa_duplicadas_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_empresa_duplicadas']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_empresa_duplicadas']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_cnpj_cpf_cond#NMF#' + search_get_sel_txt('SC_cnpj_cpf_cond') + '@NMF@';
  str_out += 'SC_cnpj_cpf#NMF#' + search_get_text('SC_cnpj_cpf') + '@NMF@';
  str_out += 'id_ac_cnpj_cpf#NMF#' + search_get_text('id_ac_cnpj_cpf') + '@NMF@';
...
<script type="text/javascript">
 $(function() {
   scClass = $("#id_ac_cnpj_cpf").attr('class').split(' ');
   scClass = scClass[ scClass.length-1 ];
   $("#id_ac_cnpj_cpf").autocomplete({
     minLength: 1,
     ...
     $.ajax({
       ...
       data: {
          ...
          nmgp_parms: "NM_ajax_opcao?#?autocomp_cnpj_cpf",
          max_itens: "10",
          cod_desc: "N",
...
<?php
   ...
   {
      if ($this->Ini->Embutida_iframe || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<?php
   ...
   {
      global 
             $cnpj_cpf_cond, $cnpj_cpf, $cnpj_cpf_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $cnpj_cpf = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']['cnpj_cpf']; 
          $cnpj_cpf_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']['cnpj_cpf_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']['NM_operador']; 
      } 
      if (!isset($cnpj_cpf_cond) || empty($cnpj_cpf_cond))
      {
         $cnpj_cpf_cond = "qp";
      }
      ...
      $str_hide_cnpj_cpf = (in_array($cnpj_cpf_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      if (!isset($cnpj_cpf) || $cnpj_cpf == "")
      {
          $cnpj_cpf = "";
      }
      if (isset($cnpj_cpf) && !empty($cnpj_cpf))
      {
         $tmp_pos = (is_string($cnpj_cpf)) ? strpos($cnpj_cpf, "##@@") : false;
         if ($tmp_pos === false)
         { }
         else
         {
         $cnpj_cpf = substr($cnpj_cpf, 0, $tmp_pos);
         }
      }
?>
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr valign="top">
      ...
      <TD id='SC_cnpj_cpf_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['cnpj_cpf'])) ? $this->New_label['cnpj_cpf'] : "CNPJ CPF"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_cnpj_cpf_cond" name="cnpj_cpf_cond" onChange="nm_campos_between(document.getElementById('id_vis_cnpj_cpf'), this, 'cnpj_cpf')">
       <OPTION value="qp" <?php if ("qp" == $cnpj_cpf_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $cnpj_cpf_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $cnpj_cpf_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $cnpj_cpf_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_cnpj_cpf" <?php echo $str_hide_cnpj_cpf?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['cnpj_cpf'])) ? $this->New_label['cnpj_cpf'] : "CNPJ CPF";
 $nmgp_tab_label .= "cnpj_cpf?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
...
<?php
      ...
      $cnpj_cpf_look = (is_string($cnpj_cpf) ? substr($this->Db->qstr($cnpj_cpf), 1, -1) : $cnpj_cpf); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct CNPJ_CPF from (SELECT E.ID, E.EMPRESA,E.CONTATO, E.CNPJ_CPF, E.DATA FROM empresa E INNER JOIN ( SELECT CNPJ_CPF, count(CNPJ_CPF) As Duplicado FROM empresa GROUP BY CNPJ_CPF HAVING count(CNPJ_CPF ) >1) TEMP on E.CNPJ_CPF = TEMP.CNPJ_CPF  ) nm_sel_esp where #lowerI#CNPJ_CPF#lowerF# = #lowerI#'$cnpj_cpf_look'#lowerF#"; 
      $this->Sel_case_sensitive($nm_comando);
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      ...
      if (isset($nmgp_def_dados[0][$cnpj_cpf]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$cnpj_cpf];
      }
      else
      {
          $sAutocompValue = $cnpj_cpf;
      }
?>
<INPUT  type="text" id="SC_cnpj_cpf" name="cnpj_cpf" value="<?php echo NM_encode_input($cnpj_cpf) ?>"  size=20 alt="{datatype: 'text', maxLength: 20, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_cnpj_cpf" name="cnpj_cpf_autocomp" size="20"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 20, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
<?php
   if (is_file("grid_empresa_duplicadas_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_empresa_duplicadas']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_empresa_duplicadas']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['orig_pesq']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.cnpj_cpf_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_cnpj_cpf'), document.F1.cnpj_cpf_cond, 'cnpj_cpf');
   document.F1.cnpj_cpf.value = "";
   document.F1.cnpj_cpf_autocomp.value = "";
 }
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "VM2025/grid_empresa_duplicadas";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['where_orig']))
      {
   ...
   function init_comando()
   {
      ...
      $this->comando         = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['where_orig'];
      $this->comando_sum     = "";
      $this->comando_filtro  = "";
   ...
   function salva_filtro($nmgp_save_origem)
   {
      global $NM_filters_save, $nmgp_save_name, $nmgp_save_option, $script_case_init;
          ...
          $NM_patch .= "grid_empresa_duplicadas/";
          if (!is_dir($NM_patch))
          {
              $NMdir = mkdir($NM_patch, 0755);
          }
   ...
   function recupera_filtro($NM_filters, $Orig_Fil)
   {
      ...
      $tp_fields['SC_cnpj_cpf_cond'] = 'cond';
      $tp_fields['SC_cnpj_cpf'] = 'text_aut';
      $tp_fields['id_ac_cnpj_cpf'] = 'text_aut';
   ...
   {
      global $cnpj_cpf_cond, $cnpj_cpf, $cnpj_cpf_autocomp, $nmgp_tab_label;

      $C_formatado = true;
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_gp_limpa.php", "F", "nm_limpa_valor") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_conv_dados.php", "F", "nm_conv_limpa_dado") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
      if (!empty($cnpj_cpf_autocomp) && empty($cnpj_cpf))
      {
          $cnpj_cpf = $cnpj_cpf_autocomp;
      }
      $cnpj_cpf_cond_salva = $cnpj_cpf_cond; 
      if (!isset($cnpj_cpf_input_2) || $cnpj_cpf_input_2 == "")
      {
          $cnpj_cpf_input_2 = $cnpj_cpf;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']['cnpj_cpf'] = $cnpj_cpf; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']['cnpj_cpf_cond'] = $cnpj_cpf_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca'] = $Temp_Busca;
      }
      if (!empty($this->Campos_Mens_erro)) 
      {
          return;
    ...
    if ($cnpj_cpf != '') {
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['cnpj_cpf'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['cnpj_cpf'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['cnpj_cpf'] = $cnpj_cpf;
      }
      $this->cmp_formatado['cnpj_cpf'] = "'" . $this->cmp_formatado['cnpj_cpf'] . "'";
      if (isset($this->cmp_formatado['cnpj_cpf_input_2'])  && !empty($this->cmp_formatado['cnpj_cpf_input_2'])) {
          $this->cmp_formatado['cnpj_cpf_input_2'] = "'" . $this->cmp_formatado['cnpj_cpf_input_2'] . "'";
      }
   }
   ...
   function processa_campos($Res_comp='pesq')
   {
      ...
      if (isset($cnpj_cpf) || $cnpj_cpf_cond == "nu" || $cnpj_cpf_cond == "nn" || $cnpj_cpf_cond == "ep" || $cnpj_cpf_cond == "ne")
      {
         $this->monta_condicao("CNPJ_CPF", $cnpj_cpf_cond, $cnpj_cpf, "", "cnpj_cpf", "VARCHAR", false);
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $this->Sel_case_sensitive($this->comando);
      $this->Sel_case_sensitive($this->comando_sum);
      $this->Sel_case_sensitive($this->comando_filtro);
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['where_pesq_filtro'] = " (" . trim($this->comando_filtro) . ")";
      }
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_empresa_duplicadas']['cond_pesq'] .= $this->NM_operador;
