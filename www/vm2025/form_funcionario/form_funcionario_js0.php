...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_funcionario']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_funcionario']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
function cep_cep(num_cep, campos_cep)
{
   ...
   jsrsExecute("form_funcionario_cep.php?cep=" + num_cep + "&onchange=s", recebe_cep, "cep", num_cep, false);
}
